/**
 * Zillow Building Price Scraper — playwright-extra + stealth
 * 
 * Install deps:
 *   npm install playwright-extra playwright-extra-plugin-stealth
 * 
 * Usage:
 *   node scraper.js --id ava-ballston
 *   node scraper.js
 *   node scraper.js --headful --id ava-ballston
 */
...
// ─── PRICE EXTRACTION ────────────────────────────────────────────────────────
// Confirmed path (via debug.js):
// props.pageProps.componentProps.initialReduxState.gdp.building.floorPlans
...
function extractPrices(nextData) {
  ...
  if (byBeds[3]) {
    ...
    result.price3_base_min = byBeds[3].baseMins.length ? Math.min(...byBeds[3].baseMins) : null;
    result.price3_base_max = byBeds[3].baseMaxs.length ? Math.max(...byBeds[3].baseMaxs) : null;
...
async function scrapeBuilding(page, building) {
  const result = {
    id: building.id, name: building.name,
    zillow_url: building.url,
    scraped_at: new Date().toISOString(),
    source: 'zillow_building_page',
  };

  try {
    await page.goto(building.url, { waitUntil: 'domcontentloaded', timeout: 30000 });
    ...
    if (title.toLowerCase().includes('access') && title.toLowerCase().includes('denied')) {
      ...
      console.log(`  ✗ Bot detection — stealth plugin may need update`);
      return result;
    }

    // Check for redirect
    const finalUrl = page.url();
    const expectedSlug = building.url.split('/').slice(-2, -1)[0];
    ...
    if (!nextData) {
      ...
      console.log(`  ✗ No __NEXT_DATA__ (title: "${title}")`);
      return result;
    }
    ...
    if (!prices) {
      ...
      result.error = 'no floorPlans at expected path';
    ...
    if (result.price2_min) {
      const base = result.price2_base_min
        ? ` (base $${result.price2_base_min.toLocaleString()}–$${result.price2_base_max.toLocaleString()})`
        : '';
      console.log(`  ✓ 2BR $${result.price2_min.toLocaleString()}–$${result.price2_max.toLocaleString()}${base}`);
      console.log(`       plans: ${result.price2_plans.join(', ')}`);
    } else {
      console.log(`  ~ No 2BR. Available: ${Object.keys(result.all_beds).join(', ')}`);
    }
    if (result.price3_min) {
      console.log(`       3BR $${result.price3_min.toLocaleString()}–$${result.price3_max.toLocaleString()}`);
    }
...
async function main() {
  ...
  if (!targets.length) {
    console.error(`Building "${singleId}" not found.\nAvailable:\n${BUILDINGS.map(b => `  ${b.id}`).join('\n')}`);
    process.exit(1);
  }

  if (dryRun) {
    targets.forEach(b => console.log(`  ${b.id.padEnd(25)} ${b.url}`));
    return;
  }
  ...
  for (let i = 0; i < targets.length; i++) {
    ...
    const result = await scrapeBuilding(page, building);
  ...
  console.log(`\n${'─'.repeat(65)}`);
  console.log(`✓ ${success} with 2BR prices   ✗ ${failed} failed/no 2BR\n`);
